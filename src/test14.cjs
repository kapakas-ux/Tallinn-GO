...
async function test() {
  ...
  const sHeader = stopsLines[0].split(';').map(h => h.trim().toUpperCase());
  const sFld = {};
  ...
  for (let i = 1; i < stopsLines.length; i++) {
    ...
    const internalId = parts[sFld['STOPID']];
    const siriId = parts[sFld['STOPCODE']];
    const name = parts[sFld['STOPNAME']];
    const latStr = parts[sFld['STOPLAT']];
    const lngStr = parts[sFld['STOPLON']];
